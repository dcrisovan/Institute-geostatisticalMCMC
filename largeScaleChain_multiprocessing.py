...
if __name__=='__main__':
    ...
    glacier_data_path = Path(r'InstituteDataGridded.csv') 
    sgs_bed_path = Path(r'sgs_bed_institute.txt')
    data_weight_path = Path(r'data_weight_institute.txt')
    seed_file_path = Path(r'../200_seeds.txt')
    output_path = Path(r'./Data/Institute')
    ...
    largeScaleChain.set_loss_type(sigma_mc=1.8, massConvInRegion=True)
    ...
    initial_beds = []
    for i in range(n_chains):
        sgs_bed = np.loadtxt('Institute_sgs_bed_'+str(i)+'.txt')
